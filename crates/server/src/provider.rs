...
/// /v1/songs/:id/url 上游获取（单路 weapi）。
/// 实验结论（2026-08-24）：网易云对数据中心出口 IP 风控（CF worker 出口 weapi/eapi/Android 姿势
/// 均返回 data[0].code=404、url=null）；仅住宅/国内云 IP 可拿 url。故不做多级 fallback
/// （徒增上游调用与延迟），空 url 由 handler 报 UPSTREAM_EMPTY_URL；真正可用需自托管到非风控 IP。
